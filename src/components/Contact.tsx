  ...
  return (
    ...
    >
      <div className="grid gap-10 lg:grid-cols-[0.9fr_1.1fr] lg:items-center">
        <div>
          <p className="text-sm uppercase tracking-[0.28em] text-[#0071e3]">Contato</p>
          <h2 className="mt-3 text-3xl font-semibold text-[#1d1d1f] sm:text-4xl">{profile.contact.title}</h2>
          <div className="mt-4 rounded-[24px] border border-[#0071e3]/10 bg-[#f7faff] p-4 text-sm text-[#1d1d1f]">
            <p className="font-medium">Se você chegou até aqui, provavelmente está procurando alguém que transforme números, processos e decisões em resultados claros.</p>
            <p className="mt-2 text-[#424245]">Eu ajudo RH, gestores e empresas a encontrar um profissional com visão executiva e presença prática.</p>
          </div>
          <p className="mt-4 max-w-xl text-[#6e6e73]">{profile.contact.description}</p>
        ...
        <div className="rounded-[28px] border border-black/5 bg-[#fafafa] p-8 shadow-[0_16px_50px_rgba(15,23,42,0.05)]">
          <form className="grid gap-5">
            ...
            <label className="space-y-2 text-sm text-[#424245]">
              ...
              <textarea
                rows={5}
                placeholder="Fale sobre a oportunidade ou projeto"
                className="w-full rounded-3xl border border-black/10 bg-white px-4 py-3 text-[#1d1d1f] outline-none transition focus:border-[#0071e3]/40 focus:ring-2 focus:ring-[#0071e3]/20"
              />
            </label>
            <label className="flex items-start gap-3 rounded-2xl border border-black/5 bg-white p-3 text-sm text-[#424245]">
              ...
              />
              <span>
                Concordo que meus dados sejam usados apenas para responder à minha mensagem e manter o contato profissional.{' '}
            ...
            {showHint && (
              <p className="text-sm text-amber-600">
                Para continuar, confirme sua autorização de uso dos dados.
              </p>
            )}
            {showConfirmation && (
              <p className="text-sm text-emerald-600">
                Obrigado! Sua mensagem será tratada com transparência e conforme a LGPD.
              </p>
            )}
            <button
              type="button"
            ...
            >
              <Send className="h-4 w-4" /> Quero conversar
            </button>
            <p className="text-xs text-[#8e8e93]">
              Este formulário não usa os dados para marketing e não armazena informações além do necessário para responder ao seu contato.
            </p>
          </form>
        </div>
      </div>
