...
export default function ArticleDetailPage() {
  ...
  useSEO({
    title: article ? article.title : 'Artigo Não Encontrado',
    description: article ? article.excerpt : 'O artigo solicitado não foi localizado em nosso portal de controladoria.'
  })
    ...
    return (
      <PageTransition>
        <section className="mx-auto max-w-xl px-6 py-32 text-center space-y-6">
          ...
          <h2 className="text-2xl font-bold text-[#1d1d1f]">Artigo não encontrado</h2>
          <p className="text-sm text-[#6e6e73]">
            O link que você seguiu pode estar quebrado ou o artigo foi removido de nossa base.
          </p>
          <Link
            to="/artigos"
      ...
      <section className="mx-auto max-w-7xl px-6 pb-24 pt-12 sm:px-8 lg:px-10">
        ...
        <div className="max-w-3xl mx-auto mb-8 flex items-center justify-between text-xs text-[#8e8e93] font-semibold border-b border-black/5 pb-4">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-[#0071e3]">{article.category}</span>
          </div>
        </div>
          ...
          <article className="bg-white rounded-[32px] border border-black/5 p-6 sm:p-8 lg:p-10 shadow-sm space-y-6">
            ...
            <div className="space-y-4">
              <span className="inline-flex items-center gap-1 rounded-full bg-[#0071e3]/5 px-3 py-1 text-xs font-bold text-[#0071e3]">
                <Bookmark className="h-3.5 w-3.5" /> {article.category}
              </span>
          ...
          <aside className="space-y-6">
            ...
            <div className="rounded-[24px] bg-[#0071e3] text-white p-6 space-y-4 shadow-sm relative overflow-hidden">
              <div className="space-y-1.5 relative z-10">
                <h4 className="text-base font-bold">Consultoria de Resultados</h4>
                <p className="text-xs text-white/80 leading-relaxed">
                  Avalie e otimize os prazos da sua empresa e implemente uma controladoria ágil orientada a dados.
                </p>
              </div>
              <Link
                to="/contato"
